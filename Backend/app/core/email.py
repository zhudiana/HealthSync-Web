...
class EmailSender:
    ...
    def send_email(
        ...
        """
        if not all([EmailConfig.SMTP_USER, EmailConfig.SMTP_PASSWORD, EmailConfig.SMTP_HOST]):
            raise HTTPException(status_code=500, detail="Email configuration not initialized")
