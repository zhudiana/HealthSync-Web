  ...
  async function loadProfile() {
    ...
    if (provider === "fitbit") {
      try {
        ...
        // If 401, try refresh once (same logic you had)
        const refreshed = await getAccessToken();
        if (!refreshed) {
          setAuth(false);
          setProfile(null);
          ...
          try {
            const data2 = await fetchProfile(refreshed, "fitbit");
            setProfile(data2?.user ?? null);
            setAuth(true);
          } catch {
            setAuth(false);
    ...
    if (provider === "withings") {
      try {
        ...
        // Don't mark unauthenticated due to a transient 4xx/5xx
        console.warn(
          "[Auth] Withings profile failed; continuing as authenticated.",
          e
        );
        setProfile((prev) => prev ?? { fullName: "Withings User" });
        setAuth(true);
        return;
      }
    }
